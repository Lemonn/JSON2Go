...
func (g *Generator) arrayGenerator(path string) ([]ast.Stmt, []string, error) {
	...
	innerStmts := []ast.Stmt{
		...
		utils.GenerateAppendStatement(levelOfArrays-1, 0, &ast.StarExpr{X: &ast.Ident{Name: string(unicode.ToLower([]rune(g.getFieldName(path))[0]))}}, &ast.Ident{Name: "result"}, "index"),
	}
		...
		Decl: &ast.GenDecl{
			...
			Specs: []ast.Spec{
				&ast.ValueSpec{
					...
					Type: utils.GeneratedNestedArray(levelOfArrays, g.getFieldType(path, 1)),
				},
			},
		},
	})
	...
	stmts = append(stmts, utils.GenerateNestedRangeStmt(levelOfArrays, innerStmts, &ast.Ident{Name: "lt"}, &ast.StarExpr{X: &ast.Ident{Name: string(unicode.ToLower([]rune(g.getFieldName(path))[0]))}}, g.getFieldType(path, 1)))
...
/*
// Handles the case where we have an array of non struct type
func (g *Generator) arrayGenerator(path string) ([]ast.Stmt, []string) {
	var stmts []ast.Stmt
	fData := g.data[path]
